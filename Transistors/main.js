...
const areCheatsEnabled = window.location.search.includes("cheats")
const g_initialStateBoost = areCheatsEnabled ? 100000000000 : 0;
...
window.onkeydown = e => {
    ...
    if (areCheatsEnabled && e.key === "F6") {
        for (let x of $(".notification-template")) {
            showNotification(x.id)
        }
    }
    if (areCheatsEnabled && e.key === "F10") {
